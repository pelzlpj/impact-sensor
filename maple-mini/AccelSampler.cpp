...
// ADC register definitions.  At present libmaple doesn't expose the
// scan-mode functionality, so we'll be setting it up by hand.
namespace ADC_CR1 {
...
// Sample at 125us intervals == 8kHz (compare to ~1kHz bandwidth of ADXL377)
...
void
AccelSampler::power_up_adc(void)
{
...
void
AccelSampler::wait_dma_complete(void)
{
    while (!dma_complete);
    dma_complete = false;
}
...
bool
AccelSampler::process_sample(void)
{
...
AccelSampler::capture_event(void)
{
    ...
    timer.refresh();
    timer.attachInterrupt(TIMER_CH1, detail::timer_isr);
    timer.resume();
